...
function initBS(){
    ...
    // jquery-ujs handler
    if (jQuery('[data-method]').length) {
        var elements = jQuery('[data-method]');

        elements.bind('ajax:success', function(e, data, status, xhr){
            var $this = jQuery(e.target);

            // if this is a delete button, remove the table row
            if ($this.is('[data-method="delete"]')) {
                $this.closest('tr').remove();
            }

            // refresh the datatable if its present
            if (typeof datatable !== 'undefined') {
                console.log(['ajax:success', 'refreshing datatable']);
